...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.titleTV = [[YYTextView alloc]initWithFrame:CGRectMake(LYMargin, 0, frame.size.width-2*LYMargin, 0)];
...
-(void)setItObj:(TRITObject *)itObj{
    ...
    if (itObj.detail.length>0) {
        self.detailTV.top = CGRectGetMaxY(self.titleTV.frame);
    ...
    if (itObj.imagePaths.count>0) {
        ...
        self.imagesView.top = self.titleTV.bounds.size.height+self.detailTV.bounds.size.height;
