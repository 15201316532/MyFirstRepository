...
@property (strong, nonatomic) TRClassesSV *classesSV;
...
-(TRClassesSV *)classesSV{
    if (!_classesSV) {
        _classesSV = [[TRClassesSV alloc]initWithFrame:CGRectMake(0, 270, LYSW, 40)];
        [self.view addSubview:_classesSV];
    }
    return _classesSV;
}
...
-(YYTextView *)titleTV{
    if (!_titleTV) {
        YYTextView *titleTV=[[YYTextView alloc]initWithFrame:CGRectMake(kMargin, 64, LYSW-2*kMargin, 30)];
...
-(YYTextView *)detailTV{
    if (!_detailTV) {
        YYTextView *detailTV=[[YYTextView alloc]initWithFrame:CGRectMake(kMargin, 115, LYSW-2*kMargin, 150)];
        detailTV.delegate = self;
        [self.view addSubview:detailTV];
        _detailTV=detailTV;
        [Utils faceMappingWithText:detailTV];
       detailTV.inputAccessoryView = self.buttonView;
        detailTV.layer.borderWidth = 1;
        detailTV.layer.borderColor = MainColor.CGColor;
    }
    return _detailTV;
}
...
-(void)loadClasses{
    ...
    [bq findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
        ...
        self.classesSV.classes = classes;
...
- (void)sendAction{
    ...
    if (self.titleTV.text.length==0||self.classesSV.selectedClasses.count==0) {
        UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"提示" message:@"信息不完整" preferredStyle:UIAlertControllerStyleAlert];
    ...
    BmobObject *bOBj = [BmobObject objectWithClassName:@"Homework"];
    [bOBj setObject:self.titleTV.text forKey:@"title"];
    [bOBj setObject:self.detailTV.text forKey:@"detail"];
    
    [bOBj setObject:self.classesSV.selectedClasses forKey:@"classes"];
