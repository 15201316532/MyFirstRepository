...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"审批作业" style:UIBarButtonItemStyleDone target:self action:@selector(homeworkAction)];
    
    [self checkingNetwork];
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"发布作业" style:UIBarButtonItemStyleDone target:self action:@selector(newHomeworkAction)];
    
    
    
    //检测未审核数量
    [NSTimer scheduledTimerWithTimeInterval:10 target:self selector:@selector(checkingUnCommentCount) userInfo:nil repeats:YES];

}
-(void)checkingUnCommentCount{
    ...
    [query countObjectsInBackgroundWithBlock:^(int number, NSError *error) {
        ...
        if (number>0) {
            self.navigationItem.leftBarButtonItem.title = [NSString stringWithFormat:@"审核作业(%d)",number];
        }
    }];
    
}

- (void)newHomeworkAction {
...
- (void)homeworkAction {
    ...
    TRCommentHomeworkViewController *vc = [TRCommentHomeworkViewController new];
