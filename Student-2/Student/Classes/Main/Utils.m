...
+(void)addScore:(int)score{
    ...
    BmobObject *user = [TRUser currentUser].bmobUser;
    
    int totalScore = [[user objectForKey:@"score"]intValue] + score;
    ...
    [user updateInBackgroundWithResultBlock:^(BOOL isSuccessful, NSError *error) {
        ...
        if (isSuccessful) {
            //更新本地用户的分数
            [user setObject:@(totalScore) forKey:@"score"];
            NSLog(@"增加积分成功！");
        }
...
+(void)addMoney:(int)money{
    ...
    int totalMoney = [[user objectForKey:@"money"]intValue] + money;
    
    [user incrementKey:@"money" byAmount:money];
    
    [user updateInBackgroundWithResultBlock:^(BOOL isSuccessful, NSError *error) {
       
        if (isSuccessful) {
            //更新本地用户的分数
            [user setObject:@(totalMoney) forKey:@"money"];
            NSLog(@"增加金币成功！");
        }
    }];
